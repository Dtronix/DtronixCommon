...
public class AllocatedOwnedMemoryPoolTests
{
    [Test]
    public void AllocatesProperBucket()
    {
        var pool = new AllocatedOwnedMemoryPool<byte>(128, 2, true);

        Assert.AreEqual(16, pool.Rent(1).Memory.Length);
        Assert.AreEqual(16, pool.Rent(16).Memory.Length);
        Assert.AreEqual(32, pool.Rent(17).Memory.Length);
        Assert.AreEqual(32, pool.Rent(32).Memory.Length);
        Assert.AreEqual(64, pool.Rent(33).Memory.Length);
        Assert.AreEqual(64, pool.Rent(64).Memory.Length);
        Assert.AreEqual(128, pool.Rent(65).Memory.Length);
        Assert.AreEqual(128, pool.Rent(128).Memory.Length);
    }

    [Test]
    public void AllocatesNextSizeUpWhenFull()
    {
        var pool = new AllocatedOwnedMemoryPool<byte>(128, 1, true);

        Assert.AreEqual(16, pool.Rent(1).Memory.Length);
        Assert.AreEqual(32, pool.Rent(1).Memory.Length);
        Assert.AreEqual(1, pool.Rent(1).Memory.Length);
    }
    ...
    {
        var pool = new AllocatedOwnedMemoryPool<byte>(16, 1, true);
